...
int main(int argc , char **argv)
{
    ...
    if(hostIP(hostname, ipstring_dest)== -1)
    {
    ...
    printf("\033[34mTraceroute a %s (%s), %d saltos max, paquetes de (%d) bytes.\n\033[92m" , hostname , ipstring_dest, MAXHOP,icmpdatabytes);
...
void setICMPHeader(unsigned char * trama, unsigned short seqNumber, int ID){
    unsigned char identifier[2];
    unsigned char checkSum[2];
    unsigned char data[31]={"Hello world my name is Bryan XD"};
    unsigned short ckSum;
...
int filterICMPreply(unsigned char *trama, int pID, unsigned short seqNumber, int tam_rcv_from, int printsetup, unsigned int ttl, struct timeval *tval_now){
    ...
    if(!memcmp (trama + 12, ETHTYPE_ICMP, 2) && !memcmp (trama + 38, identifier, 2) && !memcmp (trama + 41, sqnum, 1)){//Respuesta desde host
        ...
        if(printsetup==1){
            ...
            printf("%34s (%3d.%3d.%3d.%3d)   %10.3f ms", getHostnamefromIP(ipstring), trama[26], trama[27], trama[28], trama[29], timeres);
        }
        if(printsetup==2)
            printf("   %10.3f ms", timeres);
        if(printsetup == 3)
            printf("   %10.3f ms", timeres);
        return 1;
    }
